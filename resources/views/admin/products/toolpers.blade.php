...
@section('content')
  ...
  <div class="container">
    ...
    /
    <a href="/admin/products/systems/pers/{{ $tool->id }}" class="link-secondary route active" >پیشنیاز {{ $tool->title }}</a>
    ...
    <div class="alert alert-dark text-center"  style="width: 200px">
      ...
      <form action="/admin/products/tool/pres/create/{{ $tool->id }}" method="post">
        ...
        <table class="table table-dark" >
            ...
            @foreach ($pres as $pre)
                <tr>
                    <th scope="row">1</th>
                    <td>{{ $pre->title }}</td>
                    <td><a class="btn btn-danger" href="/admin/products/tool/pres/delete/{{ $pre->id }}">حذف پیشنیاز</a></td>
